...
class Labyrinthe{
    ...
    // Cette méthode ne doit être appelée qu'après avoir créé un labyrinthe de base par le constructeur prenant 2 entiers en paramètres - donc un labyrinthe composé seulement de murs mais avec des zones de sable toutes les 2 cases.
	...
	// voir wikipédia
	// On initialise les codes à 0 pour les cases contenant des murs, à -1 pour les autres
	int tableauFlag[] = new int[tableau.length];
	for(int i=0;i<tableauFlag.length;i++)
	    if(tableau[i].getType()==Case.MUR)
		tableauFlag[i]=0;
	    else
		tableauFlag[i]=-1;

	// On modifie les -1 par un indice permettant de numéroter toutes les cases n'étant pas des murs.
	int indice=0;
	for(int i=0;i<tableauFlag.length;i++)
	    if(tableauFlag[i]==-1)
	...
	while(!finGeneration(tableauFlag)){
		...
		tableau[posMurY*tailleX+posMurX].setType(Case.SOL);
	    }
	...
	for(int i=1;i<tailleX-1;i++)
	    for(int j=1;j<tailleY-1;j++)
		    if(getXY(i,j).getType()==Case.MUR)
		nbMurs++;
	//System.out.println("Il y a "+nbMurs+" murs sur "+(tailleX-2)*(tailleY-2)+" cases soit un pourcentage de : "+(nbMurs*1.0)/((tailleX-2)*(tailleY-2))*100.0);
    }
	...
	if(posJoueurX>tailleX-2 || posJoueurY>tailleY-2)
	    throw new java.lang.RuntimeException("Ajout d'un joueur sur une case en dehors du plateau");
	if(getXY(posJoueurX,posJoueurY).getType()!=Case.SOL)
	...
	if(getXY(listeJoueur.get(nbJoueur).getPosX()+deltaX,listeJoueur.get(nbJoueur).getPosY()+deltaY).getType()==Case.MUR)
	    System.out.println("Impossible de vous déplacer sur une case qui est un mur");
	else
	...
	return !(getXY(x,y).getType()==Case.MUR);
    }
	...
	if(typeCase==Case.TRESOR)
	    tableau[liste.get(indiceAlea)].setPointRapporte((randPlacementBonus.nextInt(9)+1)*10);
	
    }

    public void simplifierLabyrinthe(double pourcentMur){
	// Méthode qui utilise randCreationLaby instancié dans les constructeurs
	int nbCasesTotal=(tailleX-2)*(tailleY-2);
	int nbMur=(int)(nbCasesTotal*pourcentMur/100.0);
	ArrayList<Integer> liste = new ArrayList<Integer>();
	for(int i=1;i<tailleX-1;i++)
	    for(int j=1;j<tailleY-1;j++)
		if(getXY(i,j).getType()==Case.MUR)
		    liste.add(j*tailleX+i);
	while(liste.size()>nbMur){
	    int indiceAlea=randCreationLaby.nextInt(liste.size());
	    tableau[liste.get(indiceAlea)].setType(Case.SOL);
	...
	case Case.BONUS3PAS :
	    listeJoueur.get(indiceJoueur).ajouter3Pas();
	    break;
	case Case.TRESOR :
	    ...
	    break;
	case Case.BONUSSAUT :
	    listeJoueur.get(indiceJoueur).ajouterSaut();
	    break;
	default :
	    break;
	}
	getXY(listeJoueur.get(indiceJoueur).getPosX(),listeJoueur.get(indiceJoueur).getPosY()).setType(Case.SOL);
    }
	...
	if(indiceJoueur!=-1){
	    ...
	    if(eltCmd[1].equals("Bp")){
		actionTrouve=true;
		// Verifier qu'il a bien un bonus 3 pas
		if(listeJoueur.get(indiceJoueur).getNb3Pas()>0){
		    ...
		    listeJoueur.get(indiceJoueur).enlever3Pas();
		    
		}
		else{
		    System.out.println("Le joueur "+listeJoueur.get(indiceJoueur)+" n'a pas de bonus 3 pas...");
		}
	    }
	    if(eltCmd[1].equals("Bs")){
		actionTrouve=true;
		// Vérifier s'il a bien un bonus saut
		if(listeJoueur.get(indiceJoueur).getNbSaut()>0){
			...
			if(directionValide(direction)){
				...
				if(getXY(joueurPosX,joueurPosY).getType()!=Case.MUR){
				    listeJoueur.get(indiceJoueur).setPosX(joueurPosX);
				    listeJoueur.get(indiceJoueur).setPosY(joueurPosY);
				
				    ramasseEtMAJ(indiceJoueur);
				    listeJoueur.get(indiceJoueur).enleverSaut();
				}
				else{
				    System.out.println("Impossible de se déplacer sur un mur. Vous ne pouvez sauter qu'au dessus d'un simple mur. On va tout de même tenter un déplacement simple.");
			...
			System.out.println("Le joueur "+listeJoueur.get(indiceJoueur).getNom()+" n'a pas de saut");
		    }
		}
	    }
